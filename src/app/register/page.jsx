...
const Page = () => {
    ...
    return (
        <div className='p-2 m-2  flex flex-col items-center  justify-center '>
            ...
            <form className='p-4 shadow-lg border bg-white md:w-[400px] lg:w-[500px] w-auto border-slate-200' onSubmit={handleSubmit(onSubmit)}>
                <div>
                    <label className='block py-2 text-slate-500' htmlFor="">First Name</label>
                    <input id='firstname' placeholder='first name*' className='bg-zinc-50  block w-[100%] border outline-blue-500 px-2 py-1 rounded'  {...register('firstname')} />
                ...
                <div>
                    ...
                    <input placeholder='last name*' id="lastname" className='bg-zinc-50  block w-[100%] border outline-blue-500 px-2 py-1 rounded'  {...register('lastname')} />
                ...
                <div>
                    ...
                    <input placeholder='email*' id="email" className='bg-zinc-50 border  block w-[100%] outline-blue-500 px-2 py-1 rounded'  {...register('email')} />
                    {errors.email && <p className='text-red-400'>{errors.email.message}</p>}
                </div>
                <div>
                    <label className='block py-2 text-slate-500' htmlFor="password">Password</label>
                    <input type='password' placeholder='password*' id="password" className='bg-zinc-50 border block w-[100%]  outline-blue-500 px-2 py-1 rounded'  {...register('password')} />
                    {errors.password && <p className='text-red-400'>{errors.password.message}</p>}
                </div>
                <div>
                    <label className='block py-2 text-slate-500' htmlFor="confirm password">Confirm Password</label>
                    <input placeholder='Confirm password*' type='password' id="confirm password" className='bg-zinc-50 border  block w-[100%] outline-blue-500 px-2 py-1 rounded'  {...register('confirm_password')} />
                ...
                <div className='text-center m-2 p-2'>
                    <h4>Already registerd? <Link className='text-blue-500 underline' href={"/login"}>Login</Link></h4>
                </div>
            </form >
        </div >
    )
